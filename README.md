<h1 align="center">Hi 👋, I'm Aditiya Ari Wicaksono</h1>
<h3 align="center">Informatics Student | Frontend Developer | UI/UX Enthusiast</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=AAWicakzono&label=Profile%20Views&color=6A5ACD&style=flat" />
</p>

---

## 👨‍💻 About Me
🎓 Informatics student at **Universitas Muhammadiyah Surakarta (UMS)**  
🎨 Passionate about **Frontend Development & UI/UX Design**  
🚀 Love building clean, simple, and user-friendly interfaces  
📊 Experienced in dashboard & web-based system development  
🧠 Always learning new tools and best practices

---

## 🛠 Tech Stack
<p align="center">
  <img src="https://skillicons.dev/icons?i=html,css,js,tailwind,react,laravel,mysql,git,figma" />
</p>

---

## 📊 GitHub Statistics
<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=AAWicakzono&show_icons=true&theme=tokyonight&hide_border=true" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=AAWicakzono&layout=compact&theme=tokyonight&hide_border=true" />
</p>

---

## 🔥 GitHub Streak
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=AAWicakzono&theme=tokyonight&hide_border=true" />
</p>

---
<p align="center">
  <img src="https://raw.githubusercontent.com/AAWicakzono/AAWicakzono/output/github-contribution-grid-snake.svg" />
</p>


---

## 📌 Featured Projects

### 🚀 Platipus Dashboard (EO System)
Frontend dashboard for event organizer management  
**Tech:** React, Tailwind, REST API  
🔗 https://github.com/Oradra19/PlatipusFE

---

### 🏨 MiniStay — Room Booking App (Concept)
Simple daily room booking UI for kos & small hotels  
**Focus:** UX flow, clean UI, admin & user role separation  
🔗 UI Concept & Frontend Planning

---

### 🎨 UI/UX Design Practice
Case studies & interface explorations using **Figma**  
Wireframe → High-Fidelity → Prototype

---

## 🎯 Currently Learning
- Advanced React & State Management
- UI/UX Case Study Documentation
- Performance-focused Frontend
- Clean architecture for frontend projects

---

## 📫 Let's Connect
<p align="center">
  <a href="https://linkedin.com/in/USERNAME">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="mailto:email@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
  <a href="https://github.com/AAWicakzono">
    <img src="https://skillicons.dev/icons?i=github" />
  </a>
</p>

---

<p align="center">
  <i>"Simple design, strong impact."</i>
</p>
